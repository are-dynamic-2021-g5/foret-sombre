...
def tkinter_setup(model):
    ...
    # Widgets
    quitter_button = Button(button_frame, bg='white', text='Quitter', command=root.quit)
    start_button = Button(button_frame, bg='white', text='Start', command=main)
    apply_button = Button(settings_frame, bg='white', text='Apply', command=get_sliders_value)
    R_scale = Scale(settings_frame, bg='white', label='R', from_=0.1, to=5, resolution=0.1, orient=HORIZONTAL)
    fg_scale = Scale(settings_frame, bg='white', label='fg', from_=0.1, to=0.1, orient=HORIZONTAL)
    ...
    # Labels
    # Static labels
    nb_init_agents = Label(label_frame, bg='white', padx=10, text="Nombre initial de civilisation "+ str(model.nb_agents))
    ...
    label_frame.grid(row=1, column=1, sticky='nsew')
    ...
    apply_button.grid(row=5, column=0, sticky='nsew')
    ...
    fi_scale.grid(row=1, column=1, sticky='nsew')
    fc_scale.grid(row=2, column=1, sticky='nsew')
    L_scale.grid(row=3, column=1, sticky='nsew')
